...
function prepareInitialProduct(detailPayload) {
  if (!detailPayload || detailPayload.notFound) return null;
  ...
  const cabins = mergeCatalogCabins(allCabinRows, rows);
  const cabinIdMap = createCabinIdMap(allCabinRows, cabins);
  const mediaGroups = buildMediaGroups(
    detailPayload.importRows || [],
  ...
  return {
    cruise: {
      ...
      heroImage: mediaHeroImage || usableImageUrl(first.hero_image) || '',
      tags: first.tags || [],
      schedules,
    },
    cabins,
    mediaGroups,
    selectedSchedule,
    selectedCabinId: initialCabinId(cabins, selectedSchedule) || cabins[0]?.id || null,
  };
}

export default function ProductDetailClient({ id, initialDetail }) {
  const router = useRouter();
  const initialProduct = useMemo(() => prepareInitialProduct(initialDetail), [initialDetail]);
  const [loading, setLoading] = useState(!initialProduct);
  const [loadError, setLoadError] = useState('');
  const [cruise, setCruise] = useState(initialProduct?.cruise || null);
  const [cabins, setCabins] = useState(initialProduct?.cabins || []);
  const [mediaGroups, setMediaGroups] = useState(initialProduct?.mediaGroups || []);
  const [selectedCabinId, setSelectedCabinId] = useState(initialProduct?.selectedCabinId || null);
  const [detailCabinId, setDetailCabinId] = useState(null);
  const [selectedSchedule, setSelectedSchedule] = useState(initialProduct?.selectedSchedule || '');
  ...
  async function addVehicleToCart() {
    ...
    const shuttlePricePerPerson = Number(selectedVehiclePrice.price || 0);
    const shuttleTotal = passengerCount * shuttlePricePerPerson + hoanKiemSurcharge;
    ...
    const vehicleItem = {
      ...
      name: `${savedCruiseSelection.name} 차량`, optionName: `${vehicleMode === 'cruise_shuttle' ? `크루즈 셔틀 · 1인 ${formatVnd(shuttlePricePerPerson, 'VND')}` : `단독차량 · ${selectedVehiclePrice.vehicle_type}`}${hoanKiemSurcharge ? ' · 호안키엠 외 승차(+500,000 VND)' : ''}`,
    ...
    <div className="product-page">
      <div className="container product-content-wrapper">
        <main className="product-main">
          ...
          {(cruise.heroImage || archiveGroups.some((group) => group.images.length)) && (
