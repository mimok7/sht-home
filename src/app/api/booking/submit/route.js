...
async function saveRentcar(platform, owner, quoteId, item, cruiseVehicle = false, linkedCruiseReservationId = '') {
  ...
  for (const vehicle of vehicles) {
    ...
    const hoanKiemSurcharge = cruiseVehicle && data.vehicleServiceType === 'cruise_shuttle' && vehicle.hoanKiemOutsidePickup === true ? HOAN_KIEM_OUTSIDE_PICKUP_SURCHARGE : 0;
    const isCruiseShuttle = cruiseVehicle && data.vehicleServiceType === 'cruise_shuttle';
    total += isCruiseShuttle ? Math.max(1, integer(data.passengerCount, item.adults + item.children)) * CRUISE_SHUTTLE_PRICE_PER_PERSON + hoanKiemSurcharge : number(price.price) * Math.max(1, integer(vehicle.carCount, 1));
  }
  ...
  const parent = await insertParent(platform, owner.id, effectiveQuoteId, { re_type: reservationType, total_amount: total, pax_count: Math.max(1, integer(data.passengerCount, item.adults + item.children)), re_adult_count: integer(item.adults), re_child_count: integer(item.children), reservation_date: item.startDate || firstDate || null, price_breakdown: { source: 'homepage_cart', operational_details_status: 'pending', source_cruise_reservation_id: linkedCruiseReservationId || data.cruiseReservationId || null, vehicles: vehicles.map((vehicle) => ({ code: vehicle.rentcarPriceCode, count: vehicle.carCount, price: cruiseVehicle && data.vehicleServiceType === 'cruise_shuttle' ? CRUISE_SHUTTLE_PRICE_PER_PERSON : number(priceMap.get(vehicle.rentcarPriceCode)?.price), price_basis: cruiseVehicle && data.vehicleServiceType === 'cruise_shuttle' ? 'per_person' : 'per_vehicle', hoan_kiem_outside_pickup_surcharge: cruiseVehicle && data.vehicleServiceType === 'cruise_shuttle' && vehicle.hoanKiemOutsidePickup === true ? HOAN_KIEM_OUTSIDE_PICKUP_SURCHARGE : 0 })), grand_total: total } });
  const rows = vehicles.map((vehicle) => {
    const price = priceMap.get(vehicle.rentcarPriceCode);
    const count = Math.max(1, integer(vehicle.carCount, 1));
    const hoanKiemSurcharge = cruiseVehicle && data.vehicleServiceType === 'cruise_shuttle' && vehicle.hoanKiemOutsidePickup === true ? HOAN_KIEM_OUTSIDE_PICKUP_SURCHARGE : 0;
    const legacy = data.contractVersion === 1;
    if (cruiseVehicle) { const isCruiseShuttle = data.vehicleServiceType === 'cruise_shuttle'; const passengerCount = Math.max(1, integer(data.passengerCount)); return { reservation_id: parent.re_id, car_price_code: price.rent_code, rentcar_price_code: price.rent_code, way_type: vehicle.wayType, route: vehicle.route, vehicle_type: vehicle.vehicleType, rental_type: price.rental_type || null, car_count: count, passenger_count: passengerCount, pickup_datetime: legacy ? isoKst(vehicle.pickupDatetime) : null, pickup_location: legacy ? vehicle.pickupLocation || null : null, dropoff_location: legacy ? vehicle.destination || null : null, pickup_time: legacy ? vehicle.pickupTime || null : null, return_time: legacy ? vehicle.returnTime || null : null, return_datetime: legacy ? isoKst(vehicle.returnDatetime) : null, one_way_direction: vehicle.oneWayDirection || null, unit_price: isCruiseShuttle ? CRUISE_SHUTTLE_PRICE_PER_PERSON : number(price.price), car_total_price: isCruiseShuttle ? passengerCount * CRUISE_SHUTTLE_PRICE_PER_PERSON + hoanKiemSurcharge : number(price.price) * count + hoanKiemSurcharge, request_note: legacy ? data.requestNote || null : null }; }
