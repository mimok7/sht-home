...
function platformData(item) {
  ...
  if (!data || data.contractVersion !== 1) {
...
async function saveCruise(platform, owner, quoteId, item) {
  ...
  const priceBreakdown = { source: 'homepage_cart', cruise_name: data.cruiseName || item.name, schedule: data.schedule, checkin_date: checkin, room_selections: detailRows, surcharges, tour_options: data.tourOptions || [], promotion_code: promotions[0]?.code || null, promotion_name: promotions[0]?.name || null, grand_total: total };
  ...
  const rows = detailRows.map((row, index) => ({ ...row, reservation_id: parent.re_id, connecting_room: index === 0 && Boolean(data.connectingRoom), birthday_event: index === 0 && Boolean(data.birthdayEvent), birthday_name: index === 0 ? data.birthdayName || null : null, accommodation_info: index === 0 ? accommodation : null, request_note: index === 0 ? data.requestNote || null : null }));
...
async function saveHotel(platform, owner, quoteId, item) {
  ...
  const parent = await insertParent(platform, owner.id, quoteId, { re_type: 'hotel', total_amount: total, pax_count: adults + children, re_adult_count: adults, re_child_count: children, reservation_date: checkin, price_breakdown: { source: 'homepage_cart', hotel_price_code: rate.hotel_price_code, unit_price: unitPrice, room_count: roomCount, nights, grand_total: total } });
  const detail = await platform.from('reservation_hotel').insert({ reservation_id: parent.re_id, hotel_price_code: rate.hotel_price_code, schedule: `${checkin} ~ ${checkout}`, room_count: roomCount, guest_count: adults + children, checkin_date: checkin, unit_price: unitPrice, total_price: total, request_note: data.requestNote || null });
...
async function saveAirport(platform, owner, quoteId, item) {
  ...
  for (const leg of legs) {
    ...
    if (!price || !isDateValid(price, String(leg.datetime || '').slice(0, 10))) throw new Error('선택일에 적용 가능한 공항 이동 요금이 없습니다.');
  ...
  const parent = await insertParent(platform, owner.id, quoteId, { re_type: 'airport', total_amount: total, pax_count: passengers, re_adult_count: integer(item.adults), re_child_count: integer(item.children), reservation_date: String(legs[0].datetime || '').slice(0, 10), price_breakdown: { source: 'homepage_cart', legs: legs.map((leg) => ({ code: leg.airportPriceCode, way_type: leg.wayType, price: number(priceMap.get(leg.airportPriceCode)?.price) })), grand_total: total } });
  const detailRows = legs.map((leg) => { const price = priceMap.get(leg.airportPriceCode); return { reservation_id: parent.re_id, airport_price_code: price.airport_code, ra_airport_location: leg.airportLocation, accommodation_info: leg.accommodation, ra_flight_number: leg.flightNumber || '', ra_datetime: isoKst(leg.datetime), ra_passenger_count: passengers, ra_luggage_count: integer(data.luggageCount), way_type: leg.wayType, ra_car_count: 1, unit_price: number(price.price), total_price: number(price.price), request_note: data.requestNote || null }; });
...
async function saveRentcar(platform, owner, quoteId, item, cruiseVehicle = false) {
  ...
  const rows = vehicles.map((vehicle) => {
    ...
    if (cruiseVehicle) return { reservation_id: parent.re_id, car_price_code: price.rent_code, rentcar_price_code: price.rent_code, way_type: vehicle.wayType, route: vehicle.route, vehicle_type: vehicle.vehicleType, rental_type: price.rental_type || null, car_count: count, passenger_count: integer(vehicle.passengerCount), pickup_datetime: isoKst(vehicle.pickupDatetime), pickup_location: vehicle.pickupLocation || null, dropoff_location: vehicle.destination || null, pickup_time: vehicle.pickupTime || null, return_time: vehicle.returnTime || null, return_datetime: isoKst(vehicle.returnDatetime), one_way_direction: vehicle.oneWayDirection || null, unit_price: number(price.price), car_total_price: number(price.price) * count, request_note: data.requestNote || null };
    return { reservation_id: parent.re_id, rentcar_price_code: price.rent_code, pickup_datetime: isoKst(vehicle.pickupDatetime), pickup_location: vehicle.pickupLocation, destination: vehicle.destination, via_location: vehicle.viaLocation || null, via_waiting: vehicle.viaWaiting || null, return_datetime: vehicle.wayType === '편도' ? null : isoKst(vehicle.returnDatetime), return_pickup_location: vehicle.wayType === '편도' ? null : vehicle.returnPickupLocation || null, return_destination: vehicle.wayType === '편도' ? null : vehicle.returnDestination || null, return_via_location: vehicle.wayType === '편도' ? null : vehicle.returnViaLocation || null, return_via_waiting: vehicle.wayType === '편도' ? null : vehicle.returnViaWaiting || null, luggage_count: integer(data.luggageCount), passenger_count: Math.max(1, integer(data.passengerCount)), car_count: count, unit_price: number(price.price), total_price: number(price.price) * count, request_note: data.requestNote || null, way_type: vehicle.wayType };
  });
...
async function saveTour(platform, owner, quoteId, item) {
  ...
  const requestNote = [data.requestNote, data.lunchOption && `식사: ${data.lunchOption}`, data.courseOption && `코스: ${data.courseOption}`, data.nightTourOption && `야간투어: ${data.nightTourOption}`].filter(Boolean).join('\n');
  const parent = await insertParent(platform, owner.id, quoteId, { re_type: 'tour', total_amount: total, pax_count: guests, re_adult_count: integer(item.adults), re_child_count: integer(item.children), reservation_date: data.usageDate || item.startDate, price_breakdown: { source: 'homepage_cart', tour_id: data.tourId, pricing_id: rate.pricing_id, payment_method: data.paymentMethod || null, unit_price: unitPrice, addons: data.addons || [], grand_total: total } });
  const detail = await platform.from('reservation_tour').insert({ reservation_id: parent.re_id, tour_price_code: rate.pricing_id, tour_capacity: guests, pickup_location: data.pickupLocation || null, dropoff_location: data.dropoffLocation || null, usage_date: data.usageDate || item.startDate, unit_price: unitPrice, total_price: total, request_note: requestNote || null, adult_count: integer(item.adults), child_count: integer(item.children), infant_count: integer(item.infants) });
...
async function savePackage(platform, owner, item) {
  ...
  for (const packageItem of pkg.items || []) {
    ...
    if (packageItem.service_type === 'airport') {
      ...
      const legs = [
        { way_type: 'pickup', datetime: details.pickupDateTime || baseDate, airport: details.pickupAirportName || airportName, accommodation: details.accommodation || '', flight: details.flightNumber || '' },
        { way_type: 'sending', datetime: details.sandingDateTime || offsetDate(baseDate, 3), airport: details.sandingAirportName || airportName, accommodation: details.sandingPickupLocation || details.accommodation || '', flight: '' },
      ];
      const detail = await platform.from('reservation_airport').insert(legs.map((leg) => ({ reservation_id: parent.re_id, airport_price_code: 'package1', request_note: commonNote, ra_datetime: isoKst(leg.datetime), ra_flight_number: leg.flight, ra_passenger_count: guests, ra_luggage_count: guests, ra_airport_location: leg.airport, accommodation_info: leg.accommodation, way_type: leg.way_type, ra_car_count: 1, unit_price: 0, total_price: 0 })));
    ...
    if (packageItem.service_type === 'tour') {
      ...
      const detail = await platform.from('reservation_tour').insert({ reservation_id: parent.re_id, tour_price_code: pricingId, tour_capacity: guests, pickup_location: description.includes('하노이') ? '' : details.accommodation || '', dropoff_location: details.roomType || details.accommodation || '', usage_date: usageDate, adult_count: adults, child_count: children, infant_count: infants, request_note: commonNote });
      if (detail.error) throw dbError(detail.error, '패키지 투어 상세 정보를 저장하지 못했습니다.');
    }

    if (packageItem.service_type === 'hotel') {
      const detail = await platform.from('reservation_hotel').insert({ reservation_id: parent.re_id, hotel_price_code: packageItem.default_data?.hotel_price_code || null, checkin_date: usageDate, room_count: Math.max(1, integer(details.roomCount, 1)), guest_count: guests, adult_count: adults, child_count: children, infant_count: infants, accommodation_info: details.accommodation || '', request_note: commonNote });
      if (detail.error) throw dbError(detail.error, '패키지 호텔 상세 정보를 저장하지 못했습니다.');
    }

    if (packageItem.service_type === 'rentcar') {
      const detail = await platform.from('reservation_rentcar').insert({ reservation_id: parent.re_id, rentcar_price_code: packageItem.default_data?.rentcar_price_code || packageItem.default_data?.rent_code || null, pickup_datetime: usageDate, pickup_location: details.accommodation || '', destination: details.roomType || '', car_count: 1, passenger_count: guests, request_note: commonNote });
