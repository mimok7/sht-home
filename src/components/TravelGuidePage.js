...
function cruiseBudgetOptions(products, context) {
  const multiplier = Math.max(1, context.adults) * Math.max(1, context.scheduleType === 'DAY' ? 1 : context.roomCount);
  const totals = products.filter((product) => product.service_type === 'cruise').flatMap((product) => product.prices || []).filter((price) => price.currency === 'VND' && (!context.checkinDate || (!price.valid_from || price.valid_from <= context.checkinDate) && (!price.valid_to || price.valid_to >= context.checkinDate))).map((price) => Number(price.price_amount) * multiplier).filter(Number.isFinite).sort((left, right) => left - right);
  if (!totals.length) return [[null, '예산보다 조건이 중요해요', '예산 제한 없이 비교']];
  const indexes = [0.3, 0.55, 0.8, 1].map((ratio) => Math.min(totals.length - 1, Math.floor((totals.length - 1) * ratio)));
  const values = [...new Set(indexes.map((index) => Math.ceil(totals[index] / 100_000) * 100_000))];
  return [...values.map((value, index) => [value, `${value.toLocaleString('ko-KR')} VND 이하`, index === 0 ? '등록 최저가 중심으로 비교' : index === values.length - 1 ? '상위 객실까지 함께 비교' : '선택 조건의 등록 요금 범위']), [null, '예산보다 조건이 중요해요', '예산 제한 없이 비교']];
}
...
export default function TravelGuidePage() {
  ...
  const [catalog, setCatalog] = useState({ products: [], prices: [], hotelImages: [], cruiseTags: [], serviceTags: [] });
  ...
  const serviceProducts = useMemo(() => {
    ...
    for (const tag of catalog.serviceTags) {
      if (!tagsByProduct.has(tag.product_id)) tagsByProduct.set(tag.product_id, []);
      tagsByProduct.get(tag.product_id).push(tag);
    }
  ...
  useEffect(() => {
    ...
    async function loadCatalog() {
      ...
      const [productsResult, pricesResult, hotelImagesResult, cruiseTagsResult, serviceTagsResult] = await Promise.all([
        ...
        supabase.from('service_tags_v2').select('product_id,tag,evidence').eq('is_active', true).order('tag'),
      ]);
      if (cancelled) return;
      if (productsResult.error || pricesResult.error || hotelImagesResult.error || cruiseTagsResult.error || serviceTagsResult.error) {
        console.error('Failed to load public travel catalogue:', productsResult.error || pricesResult.error || hotelImagesResult.error || cruiseTagsResult.error || serviceTagsResult.error);
        setCatalogError('현재 공개 상품을 불러오지 못했습니다. 현지 데스크에 문의해 주세요.');
      } else {
        setCatalog({ products: productsResult.data || [], prices: pricesResult.data || [], hotelImages: hotelImagesResult.data || [], cruiseTags: cruiseTagsResult.data || [], serviceTags: serviceTagsResult.data || [] });
      }
  ...
  const question = (() => {
    ...
    if (currentStep === 'preferences') return <><Question title="무엇을 가장 중요하게 보고 계세요?" description="최대 2개를 중요도 순서대로 선택하세요. 첫 번째 기준의 운영 추천순위와 두 기준의 적합 점수를 함께 반영합니다." /><div className={styles.multiOptions}>{preferenceOptions.map(([value, label, evidence]) => { const selectedIndex = context.preferences.indexOf(value); return <button type="button" key={value} className={selectedIndex >= 0 ? styles.selected : ''} onClick={() => togglePreference(value)} aria-pressed={selectedIndex >= 0}><b>{label}</b><span>{selectedIndex >= 0 ? `${selectedIndex + 1}순위로 선택됨` : evidence}</span></button>; })}</div><Continue onClick={next} label={context.preferences.length ? '선택 완료' : '건너뛰기'} /></>;
...
function ServiceGuideFlow({ service, products, loading, error }) {
  ...
  const preferenceOptions = useMemo(() => [...new Map(products.flatMap((product) => product.tags).map((tag) => [tag.tag, tag.evidence])).entries()].map(([tag, evidence]) => [tag, LABELS.preference[tag] || tag.replaceAll('-', ' '), evidence]).sort(([left], [right]) => left.localeCompare(right, 'ko')), [products]);
  const steps = [...flow, ...(preferenceOptions.length ? [{ id: 'preferences', label: '추천 기준', type: 'preferences', title: '무엇을 가장 중요하게 보시나요?', description: '관리자가 추천에 활성화한 기준 중 최대 2개를 선택하면 결과 순서와 근거에 반영합니다.' }] : []), { id: 'review', label: '조건 확인', type: 'review' }];
  const [answers, setAnswers] = useState({ guests: 2, preferences: [] });
  ...
  const togglePreference = (value) => setAnswers((previous) => ({ ...previous, preferences: previous.preferences.includes(value) ? previous.preferences.filter((item) => item !== value) : previous.preferences.length < 2 ? [...previous.preferences, value] : previous.preferences }));

  let question = null;
  if (isResults) question = <ServiceCatalogResults service={service} products={matchedProducts} allProducts={products} loading={loading} error={error} onReset={() => { setAnswers({ guests: 2, preferences: [] }); setCurrentStep(steps[0]?.id || 'results'); }} />;
  else if (current?.type === 'date') question = <><Question title={current.title} description={current.description} /><div className={styles.dateChoice}><label htmlFor={`${service.id}-${current.id}`}>예정일</label><input id={`${service.id}-${current.id}`} type="date" min={new Date().toISOString().slice(0, 10)} value={answers[current.id] || ''} onChange={(event) => choose(current.id, event.target.value)} /><button type="button" className={styles.secondary} onClick={() => choose(current.id, '')}>날짜 미정</button></div></>;
  ...
  else if (current?.type === 'preferences') question = <><Question title={current.title} description={current.description} /><div className={styles.multiOptions}>{preferenceOptions.map(([value, label, evidence]) => <button type="button" key={value} className={answers.preferences.includes(value) ? styles.selected : ''} onClick={() => togglePreference(value)} aria-pressed={answers.preferences.includes(value)}><b>{label}</b><span>{answers.preferences.includes(value) ? '선택됨' : evidence}</span></button>)}</div><Continue onClick={next} label={answers.preferences.length ? '선택 완료' : '건너뛰기'} /></>;
  else if (current?.type === 'review') question = <><Question title="선택한 조건을 확인해 주세요." description="등록된 공개 상품과 활성 요금에서 적용 가능한 조건을 우선 반영해 보여드립니다." /><div className={styles.summary}>{flow.map((step) => <Summary key={step.id} label={step.label} value={serviceAnswerLabel(step, answers[step.id])} onEdit={() => setCurrentStep(step.id)} />)}{preferenceOptions.length > 0 && <Summary label="추천 기준" value={answers.preferences.map((value) => preferenceOptions.find(([tag]) => tag === value)?.[1] || value).join(', ') || '선택 안 함'} onEdit={() => setCurrentStep('preferences')} />}</div><button type="button" className={styles.recommendButton} onClick={() => setCurrentStep('results')}>{service.label} 상품 보기 <span>→</span></button></>;
...
function stepLabel(step) { return ({ schedule: '여행 일정', date: '출발일', party: '여행 인원', ages: '아동 나이', room: '객실 구성', budget: '크루즈 예산', preferences: '여행 취향', transfer: '이동 방법' })[step]; }
