...
import RecommendationPriorityPanel from './RecommendationPriorityPanel';
import HotelRecommendationPriorityPanel from './HotelRecommendationPriorityPanel';

const TAG_PRESETS = [
  { tag: 'family', evidence: '가족 객실과 아동 동반 편의 정보' }, { tag: 'couple', evidence: '발코니 또는 VIP 객실 정보' }, { tag: 'balcony', evidence: '전용 발코니 객실 정보' }, { tag: 'quiet', evidence: '조용한 휴식과 프라이빗한 일정 정보' }, { tag: 'activity', evidence: '시설 또는 특별 어메니티 정보' }, { tag: 'value', evidence: '등록 요금과 포함 사항 대비 합리적인 구성' }, { tag: 'luxury', evidence: 'VIP 객실 또는 버틀러 서비스 정보' },
  { tag: 'honeymoon', evidence: '허니문 고객에게 어울리는 객실과 특별 서비스' }, { tag: 'senior-friendly', evidence: '부모님 동반에 적합한 이동과 객실 편의' }, { tag: 'spa', evidence: '스파 또는 웰니스 프로그램 제공' }, { tag: 'fine-dining', evidence: '프리미엄 다이닝 또는 특별 식사 경험 제공' }, { tag: 'small-ship', evidence: '소규모 선박의 조용하고 여유로운 분위기' },
];
const TAGS = TAG_PRESETS.map((item) => item.tag);
...
const CRUISE_OPERATION_GROUP = {
  id: 'cruise', label: '크루즈 선택',
  items: [
    { id: 'profile', label: '기본 정보', catalogService: 'cruise', panel: 'profile' },
    { id: 'tags', label: '추천 기준', catalogService: 'cruise', panel: 'tags' },
    { id: 'recommendation-priorities', label: '추천 순위', catalogService: 'cruise', panel: 'recommendation-priorities' },
    { id: 'cabins', label: '객실 관리', catalogService: 'cruise', panel: 'cabins' },
    { id: 'rates', label: '요금 관리', catalogService: 'cruise', panel: 'rates' },
  ],
};
const HOTEL_OPERATION_GROUP = {
  id: 'hotel', label: '호텔 선택',
  items: [
    { id: 'hotel-profile', label: '기본 정보', catalogService: 'hotel', panel: 'profile' },
    { id: 'hotel-tags', label: '추천 기준', catalogService: 'hotel', panel: 'tags' },
    { id: 'hotel-recommendation-priorities', label: '추천 순위', catalogService: 'hotel', panel: 'hotel-recommendation-priorities' },
    { id: 'hotel-cabins', label: '객실 관리', catalogService: 'hotel', panel: 'cabins' },
...
function GallerySelectionControls({ images, selectedIds, busy, onToggleAll, onDeleteSelected }) {
  const allSelected = images.length > 0 && selectedIds.size === images.length;
  return <div className="gallery-selection-controls"><label className="check"><input type="checkbox" checked={allSelected} disabled={busy || images.length === 0} onChange={(event) => onToggleAll(event.target.checked)} /> 전체 선택</label><strong>{selectedIds.size}장 선택</strong><button type="button" className="admin-delete" disabled={busy || selectedIds.size === 0} onClick={onDeleteSelected}>선택 삭제</button></div>;
}

function useImageSelection(images) {
  const [selectedIds, setSelectedIds] = useState(new Set());
  const activeSelection = useMemo(() => new Set([...selectedIds].filter((id) => images.some((image) => image.id === id))), [images, selectedIds]);
  function toggle(imageId, checked) { setSelectedIds((current) => { const next = new Set(current); if (checked) next.add(imageId); else next.delete(imageId); return next; }); }
  function toggleAll(checked) { setSelectedIds(checked ? new Set(images.map((image) => image.id)) : new Set()); }
  function clear() { setSelectedIds(new Set()); }
  return { selectedIds: activeSelection, toggle, toggleAll, clear };
}

function CabinImageGallery({ images, busy, onUpload, onSetPrimary, onRemove, onRemoveSelected }) {
  const selection = useImageSelection(images);
  async function removeSelected() {
    if (!selection.selectedIds.size || !window.confirm(`선택한 객실 이미지 ${selection.selectedIds.size}장을 삭제할까요? 이 작업은 되돌릴 수 없습니다.`)) return;
...
function CruiseImageGallery({ images, busy, onSetPrimary, onRemove, onRemoveSelected }) {
  const selection = useImageSelection(images);
  async function removeSelected() {
    if (!selection.selectedIds.size || !window.confirm(`선택한 크루즈 이미지 ${selection.selectedIds.size}장을 삭제할까요? 이 작업은 되돌릴 수 없습니다.`)) return;
    await onRemoveSelected([...selection.selectedIds]); selection.clear();
  }
...
function HotelImageGallery({ images, busy, onUpload, onSetPrimary, onRemoveSelected }) {
  const selection = useImageSelection(images);
  async function removeSelected() {
    if (!selection.selectedIds.size || !window.confirm(`선택한 호텔 이미지 ${selection.selectedIds.size}장을 홈페이지에서 삭제할까요? 이 작업은 되돌릴 수 없습니다.`)) return;
    await onRemoveSelected([...selection.selectedIds]); selection.clear();
  }
...
function HotelRoomImageGallery({ roomName, images, busy, onUpload, onSetPrimary, onRemoveSelected }) {
  const selection = useImageSelection(images);
  async function removeSelected() {
    if (!selection.selectedIds.size || !window.confirm(`선택한 ${roomName} 이미지 ${selection.selectedIds.size}장을 홈페이지에서 삭제할까요? 이 작업은 되돌릴 수 없습니다.`)) return;
    await onRemoveSelected([...selection.selectedIds]); selection.clear();
  }
...
export default function AdminCruiseManager({ importOnly = false }) {
  ...
  const [cafeLoginId, setCafeLoginId] = useState('');
  const [cafeLoginPassword, setCafeLoginPassword] = useState('');
  ...
  const hotelImages = useMemo(() => (data.hotelImages || [])
    .filter((image) => image.product_id === selectedCatalogId && !image.hotel_price_code)
    .sort((left, right) => Number(right.is_primary) - Number(left.is_primary) || left.sort_order - right.sort_order || String(left.created_at).localeCompare(String(right.created_at))), [data.hotelImages, selectedCatalogId]);
  const hotelImagesByRoom = useMemo(() => {
    const grouped = new Map();
    for (const image of data.hotelImages || []) {
      if (image.product_id !== selectedCatalogId || !image.hotel_price_code) continue;
      const images = grouped.get(String(image.hotel_price_code)) || [];
      images.push(image);
      grouped.set(String(image.hotel_price_code), images);
    }
    for (const images of grouped.values()) images.sort((left, right) => Number(right.is_primary) - Number(left.is_primary) || left.sort_order - right.sort_order || String(left.created_at).localeCompare(String(right.created_at)));
    return grouped;
  }, [data.hotelImages, selectedCatalogId]);
  ...
  async function save(label, action, id, values) {
    ...
    try {
      const cloneAction = values?.__clone && action === 'updateCabin' ? 'cloneCabin' : values?.__clone && action === 'updateRate' ? 'cloneRate' : action;
      const result = await adminRequest('/api/admin/data', { method: 'PATCH', body: JSON.stringify({ action: cloneAction, id, values }) });
      setMessage(cloneAction === 'cloneRate' ? '요금 복사본을 비공개 초안으로 추가했습니다. 적용 기간을 입력한 뒤 공개해 주세요.' : cloneAction === 'cloneCabin' ? '객실 복사본을 추가했습니다. 필요한 값을 수정해 저장해 주세요.' : '저장했습니다. 공개 화면에도 변경 사항이 반영됩니다.');
      await load();
      return result;
    } catch (saveError) {
  ...
  async function uploadImages(target, entityId, selectedFiles, extra = {}) {
  ...
  async function changeHotelImage(action, imageId) {
    const label = `hotel-image-${action}-${imageId}`;
    setSaving(label); setMessage(''); setError('');
    try {
      await adminRequest('/api/admin/images', { method: 'PATCH', body: JSON.stringify({ action, imageId }) });
      setMessage(action === 'setHotelRoomPrimaryImage' ? '객실 대표 이미지를 변경했습니다.' : '호텔 대표 이미지를 변경했습니다.');
      await load();
    } catch (imageError) {
      setError(imageError.message || '호텔 대표 이미지를 변경하지 못했습니다.');
    } finally {
      setSaving('');
    }
  }

  async function removeImages(action, imageIds, label) {
    if (!imageIds.length) return;
    setSaving(`image-bulk-remove-${action}`); setMessage(''); setError('');
    try {
      const result = await adminRequest('/api/admin/images', { method: 'PATCH', body: JSON.stringify({ action, imageIds }) });
      setMessage(`${label} ${result.result?.deletedCount || imageIds.length}장을 삭제했습니다.`);
      await load();
    } catch (imageError) {
      setError(imageError.message || `${label} 삭제에 실패했습니다.`);
    } finally {
      setSaving('');
    }
  }
  ...
  async function previewCafeArticle() {
    const hasLoginId = Boolean(cafeLoginId.trim());
    const hasLoginPassword = Boolean(cafeLoginPassword);
    if (hasLoginId !== hasLoginPassword) {
      setError('로그인을 사용하려면 네이버 아이디와 비밀번호를 모두 입력해 주세요.');
      return;
    }
    setSaving('naver-preview'); setCafeImportToast(''); setMessage(''); setError('');
    try {
      const result = await adminRequest('/api/admin/naver-cafe-import', { method: 'POST', body: JSON.stringify({ action: 'preview', serviceType: cafeServiceType, url: cafeUrl, ...(hasLoginId ? { naverCredentials: { id: cafeLoginId.trim(), password: cafeLoginPassword } } : {}) }) });
  ...
  async function saveCafeImageAssignments(preview, assignmentsToSave) {
    ...
    try {
      ...
      setCafePreview((current) => {
        ...
        return {
          ...current,
          images: current.images.filter((image) => !savedImageKeys.has(sourceImageKey(image.sourceUrl))),
          imageAssignments: current.imageAssignments.filter((item) => !savedImageKeys.has(sourceImageKey(item.sourceImageUrl))),
          selectedImageUrls: current.selectedImageUrls.filter((sourceImageUrl) => !savedImageKeys.has(sourceImageKey(sourceImageUrl))),
  ...
  function saveAppliedCafeImageAssignments(nextAssignments) {
    const nextPreview = { ...cafePreview, imageAssignments: nextAssignments };
    const selected = new Set(nextPreview.selectedImageUrls);
    setCafePreview(nextPreview);
    setError('');
    void saveCafeImageAssignments(nextPreview, nextAssignments.filter((item) => selected.has(item.sourceImageUrl) && ['hero', 'cabin', 'gallery', 'hotel_room', 'hotel_menu'].includes(item.target)));
  }

  function applyCafeImageTarget(value, saveAfterApply = false) {
    ...
    if (kind === 'preset') {
      if (cafeServiceType === 'hotel') { setError('호텔 이미지는 대표·메뉴 또는 객실 갤러리로 지정해 주세요.'); return false; }
      ...
      if (saveAfterApply) saveAppliedCafeImageAssignments(next);
      else { setCafePreview({ ...cafePreview, imageAssignments: next }); setError(''); }
      return true;
    }
    if (cafeServiceType === 'hotel') {
      const hotelRoom = kind === 'hotel-room' ? cafeHotelRooms.find((room) => room.id === cabinId) : null;
      if (kind !== 'main' && kind !== 'hotel-menu' && !hotelRoom) { setError('저장할 호텔 객실 또는 메뉴 분류를 다시 선택해 주세요.'); return false; }
      const current = cafePreview.imageAssignments.filter((item) => !selected.has(item.sourceImageUrl));
      const counters = new Map();
      for (const item of current) {
        if (item.target === 'hero') counters.set('main', (counters.get('main') || 0) + 1);
        if (item.target === 'hotel_menu') counters.set('menu', (counters.get('menu') || 0) + 1);
      ...
      const next = cafePreview.imageAssignments.map((item) => {
        ...
        if (kind === 'hotel-menu') {
          const serial = (counters.get('menu') || 0) + 1;
          counters.set('menu', serial);
          return { ...item, target: 'hotel_menu', cabinId: '', hotelPriceCode: '', imageName: `menu-${String(serial).padStart(3, '0')}` };
        }
  ...
  function toggleMenuGroup(groupId) {
    setExpandedMenuGroups((current) => {
      ...
      return { 'catalog-selection': groupId === 'catalog-selection' && isOpening, cruise: groupId === 'cruise' && isOpening, hotel: groupId === 'hotel' && isOpening, requests: groupId === 'requests' && isOpening, system: groupId === 'system' && isOpening };
    });
  }
  function isMenuItemSelected(item) {
    if (item.catalogSection) return catalogService === item.catalogService && activePanel === 'catalog' && catalogSection === item.catalogSection;
  ...
  function isServiceSelected(service) {
    ...
    return ['profile', 'tags', 'recommendation-priorities', 'hotel-recommendation-priorities', 'cabins', 'rates'].includes(activePanel);
  }
  ...
  const requiresCruiseSelection = !importOnly && !['catalog', 'members', 'recommendation-priorities', 'hotel-recommendation-priorities', 'change-requests', 'change-request-status', 'change-request-completed'].includes(activePanel);
        ...
        {visibleMenuGroups.map((group) => {
          ...
          return <div className={`admin-menu-group${groupIsActive ? ' has-selected' : ''}`} key={group.id}>
            ...
            <div className="admin-menu-items" id={`admin-menu-${group.id}`} hidden={!isExpanded}>
              {group.items.map((item, index) => <button type="button" key={item.id} className={isMenuItemSelected(item) ? 'selected' : ''} onClick={() => openAdminMenu(item)}><i>{String(index + 1).padStart(2, '0')}</i>{item.label}</button>)}
            </div>
          </div>;
        })}
        ...
        {loading ? <p className="admin-loading">데이터를 불러오는 중…</p> : ((requiresCruiseSelection && !(isCatalogServiceOperation ? selectedCatalogProduct : selectedCruise)) || (activePanel === 'catalog' && !selectedCatalogProduct)) ? <p className="admin-loading">표시할 관리 데이터가 없습니다.</p> : <div className="admin-workspace" data-active={activePanel}>
          ...
          <section className="admin-section admin-panel" data-panel="profile">{isCatalogServiceOperation ? (catalogService === 'hotel' ? <HotelProfilePanel product={selectedCatalogProduct} hotelImages={hotelImages} saving={saving} onSave={(values) => save(`catalog-product-${selectedCatalogProduct.id}`, 'updateCatalogProduct', selectedCatalogProduct.id, values)} onSaveDetails={(values) => save(`catalog-details-${selectedCatalogProduct.id}`, 'updateCatalogDetails', selectedCatalogProduct.id, values)} onUpload={(files) => uploadImages('hotel-gallery', selectedCatalogProduct.id, files)} onSetPrimary={(imageId) => changeHotelImage('setHotelPrimaryImage', imageId)} onRemoveSelected={(imageIds) => removeImages('removeHotelImages', imageIds, '호텔 이미지')} /> : <StandardServiceProfilePanel product={selectedCatalogProduct} saving={saving} onSave={(values) => save(`catalog-product-${selectedCatalogProduct.id}`, 'updateCatalogProduct', selectedCatalogProduct.id, values)} onSaveDetails={(values) => save(`catalog-details-${selectedCatalogProduct.id}`, 'updateCatalogDetails', selectedCatalogProduct.id, values)} onUpload={(files) => uploadImages('catalog-hero', selectedCatalogProduct.id, files)} />) : <><div className="admin-section-title"><span>01 / CRUISE PROFILE</span><h2>기본 소개 및 공개 상태</h2><a href={selectedCruise?.slug ? `/product/${encodeURIComponent(selectedCruise.slug)}` : '#'} target="_blank" rel="noreferrer">공개 화면 보기 ↗</a></div>
            ...
            <form onSubmit={(event) => { event.preventDefault(); save('cruise', 'updateCruise', selectedId, { ...cruiseForm, star_rating: numeric(cruiseForm.star_rating) }); }} className="admin-form profile-form">
              ...
              <label className="wide">대표 이미지 경로 또는 URL<input value={cruiseForm?.hero_image || ''} onChange={(event) => setCruiseForm({ ...cruiseForm, hero_image: event.target.value })} placeholder="https:// 또는 /images/cruises/..." /></label><ImageFilePicker label="대표 이미지 선택" disabled={saving === `image-upload-cruise-hero-${selectedId}`} onSelect={(files) => uploadImages('cruise-hero', selectedId, files)} /><ImagePreview src={cruiseForm?.hero_image} alt={`${cruiseForm?.name_ko || '크루즈'} 대표 이미지`} /><CruiseImageGallery images={cruiseImages} busy={saving.startsWith('cruise-image-') || saving.startsWith('image-bulk-remove-') || saving === `image-upload-cruise-hero-${selectedId}`} onSetPrimary={(imageId) => changeCruiseImage('setCruisePrimaryImage', imageId)} onRemove={(imageId) => changeCruiseImage('removeCruiseImage', imageId)} onRemoveSelected={(imageIds) => removeImages('removeCruiseImages', imageIds, '크루즈 이미지')} /><label className="wide">상품 설명<textarea rows="5" value={cruiseForm?.description || ''} onChange={(event) => setCruiseForm({ ...cruiseForm, description: event.target.value })} /></label><label className="check wide"><input type="checkbox" checked={Boolean(cruiseForm?.is_active)} onChange={(event) => setCruiseForm({ ...cruiseForm, is_active: event.target.checked })} /> 공개 상품으로 노출</label><button className="admin-save wide" disabled={saving === 'cruise'}>{saving === 'cruise' ? '저장 중…' : '기본 정보 저장 →'}</button></form></>}</section>
          <section className="admin-section admin-panel" data-panel="naver-import">
            ...
            {cafePreview && <div className="cafe-import-preview" aria-busy={saving === 'naver-import'}>
              ...
              <div className="cafe-import-image-heading"><strong>저장/추출 대상 {cafePreview.imageAssignments.filter((item) => item.target && item.target !== 'delete').length} / {cafePreview.images.length}장</strong><small>{cafeServiceType === 'hotel' ? '대표·메뉴·객실 이미지는 여러 장을 선택해 저장할 수 있습니다. 썸네일에는 이미지 이름을 표시하지 않습니다.' : '이미지를 체크하세요. 첫 이미지 선택 후 Shift+클릭하면 연속 선택됩니다.'}</small></div>
              <div className="cafe-import-bulk cafe-import-bulk-sticky"><label className="check"><input type="checkbox" disabled={saving === 'naver-import'} checked={cafePreview.selectedImageUrls.length === cafePreview.images.length && cafePreview.images.length > 0} onChange={(event) => setCafePreview({ ...cafePreview, selectedImageUrls: event.target.checked ? cafePreview.images.map((image) => image.sourceUrl) : [], lastSelectedImageIndex: null })} /> 전체 선택</label><select aria-label="선택 이미지 저장 위치" value={cafeBulkTarget} disabled={saving === 'naver-import'} onChange={(event) => setCafeBulkTarget(event.target.value)}><option value="">선택 이미지 지정</option><option value="main">{cafeServiceType === 'hotel' ? '호텔 대표 이미지로 지정' : '대표 이미지로 지정'}</option>{cafeServiceType === 'hotel' && <option value="hotel-menu">호텔 메뉴 이미지로 지정</option>}{cafeServiceType === 'hotel' && cafeHotelRooms.map((room) => <option value={`hotel-room:${room.id}`} key={room.id}>{room.name} 객실 이미지로 지정</option>)}{cafeServiceType === 'cruise' && IMAGE_NAME_PRESETS.map((item) => <option value={`preset:${item.value}`} key={item.value}>{item.label} 이미지명 지정</option>)}{cafeServiceType === 'cruise' && cabins.filter((cabin) => !(cafePreview.hiddenCabinIds || []).includes(cabin.id)).map((cabin) => { const englishName = englishCabinName(cabin); return <option value={`cabin:${cabin.id}`} key={cabin.id} disabled={!englishName}>{englishName ? `${englishName} 객실 이미지로 지정` : `${cabin.name_ko || '객실'} · 객실명 입력 필요`}</option>; })}</select><button type="button" className="admin-save" disabled={saving === 'naver-import' || !cafeBulkTarget || !cafePreview.selectedImageUrls.length} onClick={() => { if (applyCafeImageTarget(cafeBulkTarget, true)) setCafeBulkTarget(''); }}>{saving === 'naver-import' ? <><span className="cafe-import-hourglass" aria-hidden="true" />{cafeServiceType === 'hotel' ? '호텔 이미지 저장 중…' : '크루즈 이미지 저장 중…'}</> : '이미지명 적용 후 저장'}</button><button type="button" className="admin-delete" disabled={saving === 'naver-import' || !cafePreview.selectedImageUrls.length} onClick={() => { applyCafeImageTarget('delete'); setCafeBulkTarget(''); }}>선택 이미지 삭제</button></div>
              {cafePreview.images.length > 0 && <div className="cafe-import-images">{cafePreview.images.map((image, index) => { const assignment = cafePreview.imageAssignments[index]; const imageLabel = cafeServiceType === 'hotel' ? `가져온 이미지 ${index + 1}` : assignment.imageName; return <label id={`cafe-import-image-${index}`} key={image.sourceUrl}><input type="checkbox" disabled={saving === 'naver-import'} checked={cafePreview.selectedImageUrls.includes(image.sourceUrl)} onClick={(event) => { cafeImageClickRef.current = { index, shiftKey: event.shiftKey }; }} onChange={(event) => { const click = cafeImageClickRef.current; toggleCafeImageSelection(event.target.checked, index, click.index === index && click.shiftKey); }} /><figure role="img" aria-label={imageLabel} style={{ backgroundImage: `url(${image.previewUrl})` }} />{cafeServiceType !== 'hotel' && <span>{assignment.imageName}</span>}</label>; })}</div>}
            </div>}
          </section>
          <section className="admin-section admin-panel" data-panel="tags">{isCatalogServiceOperation ? <ServiceTagPanel tags={selectedServiceTags} saving={saving} serviceLabel={CATALOG_SERVICE_LABELS[catalogService]} onSave={(tag, values) => save(`service-tag-${tag}`, 'upsertServiceTag', selectedCatalogProduct.id, { tag, ...values })} /> : <>
            <div className="admin-section-title"><span>02 / RECOMMENDATION RULES</span><h2>추천 기준 태그</h2><p>활성 태그만 여행 가이드의 고객 선호 목록과 크루즈 추천 점수에 사용됩니다.</p></div>
            <div className="tag-manager">{[...new Set([...TAGS, ...selectedTags.map((item) => item.tag)])].map((tag) => { const row = selectedTags.find((item) => item.tag === tag); const preset = TAG_PRESETS.find((item) => item.tag === tag); return <form key={tag} onSubmit={(event) => { event.preventDefault(); const values = new FormData(event.currentTarget); save(`tag-${tag}`, 'upsertCruiseTag', selectedId, { tag, evidence: values.get('evidence'), is_active: values.get('is_active') === 'on' }); }}><strong>#{tag}</strong><input name="evidence" defaultValue={row?.evidence || preset?.evidence || ''} placeholder="추천 근거를 입력하세요" required /><label className="check"><input name="is_active" type="checkbox" defaultChecked={Boolean(row?.is_active)} /> 추천에 사용</label><button disabled={saving === `tag-${tag}`}>저장</button></form>; })}</div>
            <form className="admin-form" onSubmit={(event) => { event.preventDefault(); const values = new FormData(event.currentTarget); const tag = String(values.get('tag') || '').trim(); save(`tag-new-${tag}`, 'upsertCruiseTag', selectedId, { tag, evidence: values.get('evidence'), is_active: values.get('is_active') === 'on' }); }}><label>추천 태그명<input name="tag" list="cruise-tag-suggestions" placeholder="예: honeymoon" required /></label><label>추천 근거<input name="evidence" list="cruise-tag-evidence-suggestions" placeholder="추천 이유를 선택하거나 입력하세요" required /></label><label className="check"><input name="is_active" type="checkbox" defaultChecked /> 추천에 사용</label><button className="admin-save" disabled={!selectedId || saving.startsWith('tag-new-')}>{saving.startsWith('tag-new-') ? '추가 중…' : '추천 태그 추가'}</button><datalist id="cruise-tag-suggestions">{TAG_PRESETS.map((item) => <option key={item.tag} value={item.tag} />)}</datalist><datalist id="cruise-tag-evidence-suggestions">{TAG_PRESETS.map((item) => <option key={item.tag} value={item.evidence} />)}</datalist></form>
          </>}</section>
          <section className="admin-section admin-panel" data-panel="recommendation-priorities">
            <RecommendationPriorityPanel adminRequest={adminRequest} cruises={data.cruises} itineraries={data.itineraries} tags={data.tags} operatorRole={operator?.role} />
          </section>
          <section className="admin-section admin-panel" data-panel="hotel-recommendation-priorities">
            <HotelRecommendationPriorityPanel adminRequest={adminRequest} hotels={data.catalogProducts.filter((product) => product.service_type === 'hotel')} serviceTags={data.serviceTags} operatorRole={operator?.role} />
          </section>
          ...
          <section className="admin-section admin-panel" data-panel="cabins">{isCatalogServiceOperation ? (catalogService === 'hotel' ? <HotelCabinPanel rooms={hotelRooms} imagesByRoom={hotelImagesByRoom} productId={selectedCatalogProduct.id} saving={saving} onUpload={(roomCode, files) => uploadImages('hotel-room-gallery', selectedCatalogProduct.id, files, { hotelPriceCode: roomCode })} onSetPrimary={(imageId) => changeHotelImage('setHotelRoomPrimaryImage', imageId)} onRemoveSelected={(imageIds) => removeImages('removeHotelImages', imageIds, '호텔 객실 이미지')} /> : <StandardServiceConfigurationPanel serviceLabel={CATALOG_SERVICE_LABELS[catalogService]} details={selectedServiceDetails} />) : <><div className="admin-section-title"><span>04 / CABINS</span><h2>객실 정보</h2><p>객실의 특징은 추천 태그와 고객 안내에 사용됩니다.</p></div><div className="cabin-add-action"><button type="button" className="admin-save" onClick={() => { void createCabin(); }} disabled={saving === 'cabin-create'}>{saving === 'cabin-create' ? '객실 추가 중…' : '객실 추가'}</button></div><div className="cabin-editor">{cabins.map((cabin) => <CabinForm key={cabin.id} cabin={cabin} images={cabinImagesByCabin.get(cabin.id) || []} saving={saving === `cabin-${cabin.id}`} imageBusy={saving === `image-upload-cabin-gallery-${cabin.id}` || saving.startsWith('cabin-image-') || saving.startsWith('image-bulk-remove-')} onSave={(values) => save(`cabin-${cabin.id}`, 'updateCabin', cabin.id, values)} onUpload={(files) => uploadImages('cabin-gallery', cabin.id, files)} onSetPrimary={(imageId) => changeCabinImage('setCabinPrimaryImage', imageId)} onRemove={(imageId) => changeCabinImage('removeCabinImage', imageId)} onRemoveSelected={(imageIds) => removeImages('removeCabinImages', imageIds, '객실 이미지')} />)}</div></>}</section>
          ...
          <section className="admin-section admin-panel" data-panel="catalog">
            ...
            <div className="catalog-select-panel">
              <label>서비스<select value={catalogService} onChange={(event) => selectCatalogService(event.target.value)}>{[['tour', '투어'], ['vehicle', '차량'], ['airport', '공항']].map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
...
function CabinForm({ cabin, images, onSave, onUpload, onSetPrimary, onRemove, onRemoveSelected, saving, imageBusy }) {
  ...
  return <form onSubmit={(event) => { event.preventDefault(); onSave({ name_ko: form.name_ko, name_en: form.name_en || null, image_url: form.image_url || null, room_area_text: form.room_area_text || null, bed_type: form.bed_type || null, max_adults: numeric(form.max_adults), max_guests: numeric(form.max_guests), has_balcony: form.has_balcony, is_vip: form.is_vip, has_butler: form.has_butler, is_recommended: form.is_recommended, connecting_available: form.connecting_available, extra_bed_available: form.extra_bed_available, facilities: form.facilities || null, special_amenities: form.special_amenities || null, is_active: form.is_active }); }}>
    ...
    <div className="cabin-grid">
      ...
      <CabinImageGallery images={images} busy={imageBusy} onUpload={onUpload} onSetPrimary={onSetPrimary} onRemove={onRemove} onRemoveSelected={onRemoveSelected} />
    ...
    <button type="button" className="admin-save secondary" disabled={saving} onClick={() => { const nameKo = window.prompt('복사할 새 객실명을 입력해 주세요.', `${form.name_ko || cabin.name_ko} 복사본`); if (nameKo?.trim()) onSave({ name_ko: nameKo, name_en: form.name_en || null, image_url: form.image_url || null, room_area_text: form.room_area_text || null, bed_type: form.bed_type || null, max_adults: numeric(form.max_adults), max_guests: numeric(form.max_guests), has_balcony: form.has_balcony, is_vip: form.is_vip, has_butler: form.has_butler, is_recommended: form.is_recommended, connecting_available: form.connecting_available, extra_bed_available: form.extra_bed_available, facilities: form.facilities || null, special_amenities: form.special_amenities || null, is_active: form.is_active, __clone: true }); }}>{saving ? '처리 중…' : '이 객실 복사하여 추가'}</button>
  </form>;
}
...
function HotelProfilePanel({ product, hotelImages, saving, onSave, onSaveDetails, onUpload, onSetPrimary, onRemoveSelected }) {
  const imageBusy = saving === `image-upload-hotel-gallery-${product.id}` || saving.startsWith('hotel-image-setHotelPrimaryImage-') || saving.startsWith('image-bulk-remove-');
  return <><div className="admin-section-title"><span>01 / HOTEL PROFILE</span><h2>기본 소개 및 공개 상태</h2><p>호텔 소개, 공개 상태, 위치·등급, 대표 이미지를 한 화면에서 관리합니다.</p></div>
    <CatalogProductForm product={product} saving={saving === `catalog-product-${product.id}`} imageBusy={imageBusy} onSave={onSave} onUpload={onUpload} />
    <CatalogDetailsForm product={product} saving={saving === `catalog-details-${product.id}`} onSave={onSaveDetails} />
    <HotelImageGallery images={hotelImages} busy={imageBusy} onUpload={onUpload} onSetPrimary={onSetPrimary} onRemoveSelected={onRemoveSelected} />
  </>;
}

function ServiceTagPanel({ tags, saving, serviceLabel = '호텔', onSave }) {
  const availableTags = [...new Set([...TAGS, ...tags.map((item) => item.tag)])];
  return <><div className="admin-section-title"><span>02 / RECOMMENDATION RULES</span><h2>추천 기준 태그</h2><p>크루즈와 동일하게 {serviceLabel}별 추천 기준과 근거를 관리합니다.</p></div><div className="tag-manager">{availableTags.map((tag) => {
    const row = tags.find((item) => item.tag === tag);
    const preset = TAG_PRESETS.find((item) => item.tag === tag);
    return <form key={tag} onSubmit={(event) => { event.preventDefault(); const values = new FormData(event.currentTarget); onSave(tag, { evidence: values.get('evidence'), is_active: values.get('is_active') === 'on' }); }}><strong>#{tag}</strong><input name="evidence" defaultValue={row?.evidence || preset?.evidence || ''} placeholder="추천 근거를 입력하세요" required /><label className="check"><input name="is_active" type="checkbox" defaultChecked={Boolean(row?.is_active)} /> 추천에 사용</label><button disabled={saving === `service-tag-${tag}`}>{saving === `service-tag-${tag}` ? '저장 중…' : '저장'}</button></form>;
  })}</div><form className="admin-form" onSubmit={(event) => { event.preventDefault(); const values = new FormData(event.currentTarget); const tag = String(values.get('tag') || '').trim(); onSave(tag, { evidence: values.get('evidence'), is_active: values.get('is_active') === 'on' }); }}><label>추천 태그명<input name="tag" list="service-tag-suggestions" placeholder="예: senior-friendly" required /></label><label>추천 근거<input name="evidence" list="service-tag-evidence-suggestions" placeholder="추천 이유를 선택하거나 입력하세요" required /></label><label className="check"><input name="is_active" type="checkbox" defaultChecked /> 추천에 사용</label><button className="admin-save" disabled={saving.startsWith('service-tag-')}>{saving.startsWith('service-tag-') ? '저장 중…' : '추천 태그 추가'}</button><datalist id="service-tag-suggestions">{TAG_PRESETS.map((item) => <option key={item.tag} value={item.tag} />)}</datalist><datalist id="service-tag-evidence-suggestions">{TAG_PRESETS.map((item) => <option key={item.tag} value={item.evidence} />)}</datalist></form></>;
}

function HotelCabinPanel({ rooms, imagesByRoom, productId, saving, onUpload, onSetPrimary, onRemoveSelected }) {
  return <><div className="admin-section-title"><span>04 / CABINS</span><h2>객실 정보</h2><p>객실별 기본 정보와 이미지 갤러리를 관리합니다. 객실 원본 정보는 플랫폼과 동기화됩니다.</p></div>{rooms.length === 0 ? <p className="admin-loading">동기화된 객실 데이터가 없습니다.</p> : <div className="rate-editor">{rooms.map(({ detail, room }) => <HotelRoomForm key={`${detail.product_id}-${detail.source_id}`} room={room} images={imagesByRoom.get(String(detail.source_id)) || []} showPrice={false} imageBusy={saving === `image-upload-hotel-room-gallery-${productId}` || saving.startsWith('hotel-image-setHotelRoomPrimaryImage-') || saving.startsWith('image-bulk-remove-')} onUpload={(files) => onUpload(String(detail.source_id), files)} onSetPrimary={onSetPrimary} onRemoveSelected={onRemoveSelected} />)}</div>}</>;
}

function HotelRatePanel({ rooms, expandedRoomId, onToggle, saving, onSave }) {
  return <><div className="admin-section-title"><span>05 / RATES</span><h2>등록 요금 및 적용 기간</h2><p>객실명을 누르면 해당 객실의 홈페이지 요금만 펼쳐집니다. 금액은 VND 기준이며, 공개 여부를 끄면 고객 화면에서 제외됩니다.</p></div><div className="rate-cabin-groups">{rooms.map(({ id, room, price }, index) => {
    const isExpanded = expandedRoomId === id;
    const panelId = `hotel-rate-room-panel-${id}`;
...
function HotelRoomForm({ room, price, images, onSave, onUpload, onSetPrimary, onRemoveSelected, saving, imageBusy, showPrice = true }) {
  const roomName = room.room_name || room.room_type || '객실명 없음';
  const detailItems = [
  ...
  return <article className="hotel-room-card"><header><div><span>ROOM / {room.hotel_price_code || '-'}</span><b>{roomName}</b></div><small>{room.hotel_name || ''}</small></header><dl className="hotel-room-details">{detailItems.map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl>{room.notes && <p className="hotel-room-notes">{room.notes}</p>}<HotelRoomImageGallery roomName={roomName} images={images} busy={imageBusy} onUpload={onUpload} onSetPrimary={onSetPrimary} onRemoveSelected={onRemoveSelected} />{showPrice && (price ? <CatalogPriceForm price={price} onSave={onSave} saving={saving} /> : <p className="admin-notice error">이 객실의 홈페이지 요금 레코드를 찾지 못했습니다. 플랫폼 동기화를 다시 실행해 주세요.</p>)}</article>;
}
