...
export default function ReservationDocumentsPage() {
  ...
  return <div className="booking-page"><div className="booking-shell">
    ...
    <div className="booking-title-row"><div><span className="booking-section-kicker">TRAVEL DOCUMENTS</span><h1>여권 · 승선코드</h1></div><span className="beta-badge">PLATFORM SHARED DATA</span></div>
    ...
    {state.loading && <div className="booking-empty"><h2>여행 서류를 확인하고 있습니다.</h2><p>플랫폼 DB에서 본인 크루즈 예약의 여권과 승선코드를 조회합니다.</p></div>}
    ...
    {!state.loading && !state.error && <>
      <section className="booking-panel document-upload-panel"><div className="booking-panel-head"><span>PASSENGER PASSPORTS</span><h2>탑승객 여권</h2><p>여러 명의 여권 사진을 한 번에 선택해 등록할 수 있습니다. 등록 자료는 플랫폼 DB에 저장됩니다.</p></div><div className="booking-panel-body"><div className="document-upload-actions"><div><strong>{state.passports.length}장 등록됨</strong><p>{state.cruises.length ? `연결 예약: ${state.cruises[0].rate?.cruise_name || '크루즈'} · ${formatDate(state.cruises[0].checkin)}` : '등록 가능한 크루즈 예약이 없습니다.'}</p></div><input ref={fileInput} className="visually-hidden" type="file" accept="image/jpeg,image/png,image/webp" multiple onChange={uploadPassports} /><button className="booking-action primary" type="button" disabled={state.saving || !state.cruises.length} onClick={() => fileInput.current?.click()}>{state.saving ? '저장 중…' : '여권 추가하기'}</button></div>{state.notice && <p className="booking-sync-note">{state.notice}</p>}{state.passports.length > 0 && <div className="document-image-grid">{state.passports.map((document, index) => <a key={document.id} href={document.image_data} target="_blank" rel="noreferrer" className="document-image-card"><img src={document.image_data} alt={`탑승객 여권 ${index + 1}`} /><span>여권 {index + 1} · {formatDate(document.created_at)}</span></a>)}</div>}</div></section>
      <section className="document-cruise-list"><div className="document-section-heading"><span>BOARDING INFORMATION</span><h2>크루즈 승선코드</h2><p>매니저가 플랫폼에서 등록한 승선코드와 이미지를 표시합니다.</p></div>{state.cruises.length === 0 && <div className="booking-empty"><h2>크루즈 예약이 없습니다.</h2><p>크루즈 예약이 생성되면 여권 등록 및 승선코드 확인이 가능해집니다.</p></div>}{state.cruises.map((cruise) => <article className="booking-panel document-cruise-card" key={cruise.reservation_id}><div className="booking-panel-head"><span>{formatDate(cruise.checkin)}</span><h2>{cruise.rate?.cruise_name || '크루즈 예약'}</h2><p>{cruise.rate?.room_type || '객실 정보 확인 중'} · 예약번호 {String(cruise.reservation_id).slice(0, 8).toUpperCase()}</p></div><div className="booking-panel-body"><dl className="document-code-grid"><div><dt>승선 코드</dt><dd>{cruise.boarding_code || '매니저 등록 대기'}</dd></div><div><dt>승선일</dt><dd>{formatDate(cruise.checkin)}</dd></div></dl>{cruise.boardingImages.length > 0 ? <div className="document-image-grid boarding-images">{cruise.boardingImages.map((document, index) => <a key={document.id} href={document.image_data} target="_blank" rel="noreferrer" className="document-image-card"><img src={document.image_data} alt={`${cruise.rate?.cruise_name || '크루즈'} 승선코드 ${index + 1}`} /><span>승선코드 이미지 보기 ↗</span></a>)}</div> : <p className="document-pending">승선코드 이미지는 매니저가 확정 후 등록하면 이곳에 표시됩니다.</p>}</div></article>)}</section>
    </>}
    <div className="booking-warning">여권과 승선코드는 여행 운영 목적으로 플랫폼 DB에서 조회합니다. 여권 이미지는 크루즈 일정에 연결되어 보관되며, 승선코드는 매니저가 등록한 정보만 표시합니다.</div>
  </div></div>;
}
