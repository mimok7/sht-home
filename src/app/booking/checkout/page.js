...
export default function BookingCheckoutPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (!cart.synced || cart.status && cart.status !== 'active') { setState({ loading: false, user: auth.user, items, payments: [], error: cart.error || '예약 처리 상태를 확인해 주세요. 중복 접수하지 말고 예약 내역을 확인해 주세요.', complete: null }); return; }
      ...
      if (!cancelled) setState({ loading: false, user: auth.user, items, cartUpdatedAt: cart.updatedAt, payments: paymentResult.data || [], error: paymentResult.error ? '결제 상태를 확인하지 못했습니다.' : '', complete: null });
    }
  ...
  async function submitCart() {
    if (!state.items.length || submitting || !state.cartUpdatedAt) return;
    setSubmitting(true);
    ...
    try {
      ...
      const response = await fetch('/api/booking/submit', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${session.access_token}` }, body: JSON.stringify({ updatedAt: state.cartUpdatedAt }) });
      const result = await response.json();
      if (!response.ok) {
        const cart = await hydrateBookingCart();
        setState((current) => ({ ...current, items: cart.items, cartUpdatedAt: cart.synced && cart.status === 'active' ? cart.updatedAt : null }));
        throw new Error(result.error || '장바구니 예약을 저장하지 못했습니다.');
      }
      writeBookingCart([]);
  ...
  return <div className="booking-page"><div className="booking-shell checkout-shell">
    ...
    <div className="booking-title-row"><div><span className="booking-section-kicker">FINAL REVIEW</span><h1>통합 예약·결제</h1></div><span className="beta-badge">신규 예약 · 베타</span></div>
    {state.loading ? <div className="booking-empty"><h2>결제 정보를 확인하고 있습니다.</h2><p>홈페이지 장바구니와 예약 소유권, 결제 가능 금액을 조회합니다.</p></div> : state.complete ? <div className="booking-empty booking-submit-complete"><span className="booking-section-kicker">RESERVATION RECEIVED</span><h2>{state.complete.itemCount}개 서비스를 저장했습니다.</h2><p>예약이 접수되었습니다. 담당자가 예약 가능 여부와 최종 금액을 확인한 뒤 결제 링크를 안내합니다.</p>{state.error && <p className="booking-error">{state.error}</p>}<div className="booking-controls"><Link className="secondary" href="/booking/reservations">예약 내역 확인 →</Link></div></div> : <div className="checkout-grid">
      ...
      <aside className="booking-panel"><div className="booking-panel-head"><h2>예약 접수와 결제 안내</h2></div><div className="booking-panel-body">{state.error && <p className="booking-error">{state.error}</p>}{pendingPayments.length > 0 && <div className="checkout-payable"><span>기존 결제 대기 합계</span><strong>{pendingTotal.toLocaleString('ko-KR')} VND</strong></div>}<div className="booking-warning">예약 접수 시 최신 요금을 다시 확인합니다. 최종 예약 가능 여부와 결제 금액은 담당자가 확인한 뒤 안내합니다.</div><div className="booking-controls"><Link className="secondary" href="/booking/reservations">예약 내역 확인 →</Link><button type="button" disabled={!state.items.length || submitting || !state.cartUpdatedAt} onClick={submitCart}>{submitting ? '예약 접수 중…' : '예약 접수하기 →'}</button><a href="http://pf.kakao.com/_zvsxaG/chat" target="_blank" rel="noreferrer" className="secondary">예약 문의 ↗</a></div></div></aside>
    </div>}
  </div></div>;
}
