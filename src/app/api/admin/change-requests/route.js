...
export async function GET(request) { try { const { database } = await operatorAndDatabase(request); return Response.json({ requests: await list(database) }); } catch (error) { return fail(error); } }
