...
function CruiseImageGallery({ images, initialImageId, busy, onSetPrimary, onSetInitial, onRemove, onRemoveSelected }) {
  const selection = useImageSelection(images);
  async function removeSelected() {
  ...
  return <section className="cabin-image-gallery wide" aria-label="크루즈 업로드 이미지 관리">
    <div className="cabin-image-gallery-heading"><div><span>CRUISE GALLERY</span><strong>업로드 이미지 {images.length}장</strong><small><b>초기 이미지</b>는 첫 화면 카드에만 한 장 노출됩니다. <b>대표 이미지</b>는 상세 화면의 기본 이미지입니다.</small></div></div>
    {images.length > 0 ? <GalleryPreview imageCount={images.length}><GallerySelectionControls images={images} selectedIds={selection.selectedIds} busy={busy} onToggleAll={selection.toggleAll} onDeleteSelected={removeSelected} /><div className="cabin-image-grid">{images.map((image, index) => {
      const imageUrl = storedImageUrl(image);
      return <ImageSurface key={`${image.id}-${imageUrl}`} className={`cabin-gallery-image ${selection.selectedIds.has(image.id) ? 'is-selected' : ''}`} src={imageUrl} alt={image.image_name || `크루즈 이미지 ${index + 1}`}><label className="gallery-image-select"><input type="checkbox" checked={selection.selectedIds.has(image.id)} disabled={busy} onChange={(event) => selection.toggle(image.id, event.target.checked)} /> 선택</label>
        <figcaption><span>{image.id === initialImageId ? '초기 이미지 · 첫 화면' : image.is_primary ? '대표 이미지 · 상세 화면' : `이미지 ${index + 1}`}</span><div>{image.id !== initialImageId && <button type="button" onClick={() => onSetInitial(image.id)} disabled={busy}>초기 이미지로 지정</button>}{!image.is_primary && <button type="button" onClick={() => onSetPrimary(image.id)} disabled={busy}>대표로 지정</button>}<button type="button" className="danger" onClick={() => onRemove(image.id)} disabled={busy}>삭제</button></div></figcaption>
      </ImageSurface>;
...
export default function AdminCruiseManager({ importOnly = false }) {
  ...
  async function changeCruiseImage(action, imageId) {
    ...
    try {
      await adminRequest('/api/admin/images', { method: 'PATCH', body: JSON.stringify({ action, imageId }) });
      setMessage(action === 'setCruiseInitialImage' ? '초기 이미지를 변경했습니다. 첫 화면 카드에는 이 이미지 한 장만 표시됩니다.' : action === 'setCruisePrimaryImage' ? '대표 이미지를 변경했습니다.' : '크루즈 이미지를 삭제했습니다.');
      await load();
    } catch (imageError) {
