...
export default function BookingCheckoutPage() {
  ...
  async function startOnepayPayment(reservationIds, session, idempotencyKey) {
    const paymentResponse = await fetch('/api/payments/onepay/create', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${session.access_token}` }, body: JSON.stringify({ reservationIds, idempotencyKey }) });
    const payment = await paymentResponse.json();
    if (!paymentResponse.ok || !payment.url) throw new Error(payment.error || '원페이 결제를 시작하지 못했습니다.');
  ...
  async function submitCart() {
    ...
    try {
      ...
      await startOnepayPayment(result.reservationIds, session, idempotencyKey);
    } catch (error) {
      setState((current) => ({ ...current, error: error.message || '장바구니 예약을 저장하지 못했습니다.' }));
    } finally { setSubmitting(false); }
  }

  async function retryOnepayPayment() {
    if (!state.complete?.reservationIds?.length || submitting) return;
    setSubmitting(true);
    setState((current) => ({ ...current, error: '' }));
    try {
      const session = await getPlatformCartSession();
      ...
      setPaymentIdempotencyKey(idempotencyKey);
      await startOnepayPayment(state.complete.reservationIds, session, idempotencyKey);
    } catch (error) {
      setState((current) => ({ ...current, error: error.message || '원페이 결제를 시작하지 못했습니다.' }));
    } finally { setSubmitting(false); }
  }
  ...
  return <div className="booking-page"><div className="booking-shell checkout-shell">
    ...
    {state.loading ? <div className="booking-empty"><h2>결제 정보를 확인하고 있습니다.</h2><p>홈페이지 장바구니와 예약 소유권, 결제 가능 금액을 조회합니다.</p></div> : state.complete ? <div className="booking-empty booking-submit-complete"><span className="booking-section-kicker">PAYMENT START</span><h2>{state.complete.itemCount}개 서비스를 저장했습니다.</h2><p>원페이 결제 페이지로 이동하지 못했습니다. 아래 버튼으로 같은 결제를 다시 시작할 수 있습니다.</p>{state.error && <p className="booking-error">{state.error}</p>}<div className="booking-controls"><button type="button" disabled={submitting} onClick={retryOnepayPayment}>{submitting ? '결제 준비 중…' : '원페이 결제 다시 시도 →'}</button><Link className="secondary" href="/booking/reservations">예약 내역 확인 →</Link></div></div> : <div className="checkout-grid">
