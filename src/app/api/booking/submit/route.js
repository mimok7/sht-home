...
async function saveRentcar(platform, owner, quoteId, item, cruiseVehicle = false, linkedCruiseReservationId = '') {
  ...
  for (const vehicle of vehicles) {
    ...
    total += isCruiseShuttle ? Math.max(1, integer(data.passengerCount, item.adults + item.children)) * number(price.price) + hoanKiemSurcharge : number(price.price) * Math.max(1, integer(vehicle.carCount, 1));
  }
  ...
  const rows = vehicles.map((vehicle) => {
    ...
    if (cruiseVehicle) { const isCruiseShuttle = data.vehicleServiceType === 'cruise_shuttle'; const passengerCount = Math.max(1, integer(data.passengerCount)); return { reservation_id: parent.re_id, car_price_code: price.rent_code, rentcar_price_code: price.rent_code, way_type: vehicle.wayType, route: vehicle.route, vehicle_type: vehicle.vehicleType, rental_type: price.rental_type || null, car_count: count, passenger_count: passengerCount, pickup_datetime: legacy ? isoKst(vehicle.pickupDatetime) : null, pickup_location: legacy ? vehicle.pickupLocation || null : null, dropoff_location: legacy ? vehicle.destination || null : null, pickup_time: legacy ? vehicle.pickupTime || null : null, return_time: legacy ? vehicle.returnTime || null : null, return_datetime: legacy ? isoKst(vehicle.returnDatetime) : null, one_way_direction: vehicle.oneWayDirection || null, unit_price: number(price.price), car_total_price: isCruiseShuttle ? passengerCount * number(price.price) + hoanKiemSurcharge : number(price.price) * count + hoanKiemSurcharge, request_note: legacy ? data.requestNote || null : null }; }
