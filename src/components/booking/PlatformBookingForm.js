...
const SERVICES = {
  ...
  cruise_vehicle: { label: '크루즈 차량만 추가', example: '크루즈 차량' },
...
export default function PlatformBookingForm({ type }) {
  ...
  return <div className="booking-page"><div className="booking-shell"><Link href="/booking" className="booking-back">← 전체 서비스</Link><div className="booking-title-row"><div><span className="booking-section-kicker">CUSTOMER APP DATA / HOMEPAGE DESIGN</span><h1>{service.label}</h1></div><BookingCartLink className="beta-badge" header={false}>장바구니</BookingCartLink></div>{loading ? <div className="booking-empty"><h2>플랫폼 상품을 불러오는 중입니다.</h2><p>고객앱과 같은 목록과 최신 요금을 확인합니다.</p></div> : <form className="service-flow" onSubmit={addToCart}><section className="booking-panel"><div className="booking-panel-head"><span>01 / RESERVATION DETAILS</span><h2>여행 조건 선택</h2><p>목록과 연속 필터는 플랫폼 고객앱과 동일한 원본 데이터를 사용합니다.</p></div><div className="booking-panel-body">{fields()}{type === 'package' && packageItemFields()}<div className="booking-field full booking-request-note"><label>요청사항</label><textarea value={form.requestNote} onChange={(event) => set('requestNote', event.target.value)} placeholder="필요한 요청사항을 입력해 주세요." /></div></div></section><aside className="service-selection-summary"><div><span>02 / SELECTION SUMMARY</span><h2>선택 내용</h2></div><dl>{summaryRows().map(([label, value]) => <div key={label}><dt>{label}</dt><dd className={label.includes('금액') ? 'booking-summary-price' : ''}>{value}</dd></div>)}</dl><p>장바구니는 홈페이지 DB에 저장됩니다. 최종 예약 저장 시 플랫폼 원본 요금과 코드가 다시 검증되고 고객앱과 같은 예약 상세 테이블에 생성됩니다.</p></aside>{error && <p className="booking-error" role="alert">{error}</p>}{message && <p className="booking-warning" role="status">{message}</p>}<div className="booking-controls"><button type="submit" disabled={saving}>{saving ? '홈페이지 DB 저장 중…' : editingCartItemId ? '장바구니 수정 저장 →' : '장바구니에 저장 →'}</button><BookingCartLink className="secondary" showCount={false} header={false}>장바구니 보기 →</BookingCartLink></div></form>}</div></div>;
}
