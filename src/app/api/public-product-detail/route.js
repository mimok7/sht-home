...
async function loadCruiseDetail(id) {
  ...
  const cabinImagesPromise = cabinIds.length
    ? supabase
...
async function loadPublicProductDetail(service, id) {
  return service === 'cruise' ? loadCruiseDetail(id) : loadHotelDetail(id);
}

const getCachedPublicProductDetail = unstable_cache(
  loadPublicProductDetail,
  ['public-product-detail-v2'],
...
export async function GET(request) {
  ...
  try {
    const detail = await getCachedPublicProductDetail(service, id);
