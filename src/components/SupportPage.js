...
import masthead from './SupportPageMasthead.module.css';
...
export default function SupportPage({ eyebrow, title, intro, children }) {
  return <div className={styles.page}>
    <header className={`${styles.hero} ${masthead.hero}`}><div className={`${styles.heroInner} ${masthead.heroInner}`}><p className={styles.eyebrow}>{eyebrow}</p><h1>{title}</h1><p className={styles.intro}>{intro}</p></div></header>
